...
try:
    ...
    while True:
        ...
        # Convert trash height (cm) to volume (L) using (height / 34 cm) * 30L
        trash_volumes = {
            "paper": round((trash_levels["paper"] / 34) * 30, 2),
            "plastic": round((trash_levels["plastic"] / 34) * 30, 2),
            "metal": round((trash_levels["metal"] / 34) * 30, 2)
        }

        print("Trash Volumes - Paper: {:.2f}L, Plastic: {:.2f}L, Metal: {:.2f}L".format(
            trash_volumes["paper"], trash_volumes["plastic"], trash_volumes["metal"]
        ))

        send_three_bin_data(trash_volumes)  # Send volume data instead of raw height
